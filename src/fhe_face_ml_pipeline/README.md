...
Performs bounding box regression on an input image using a trained face model wrapped by an encrypted FHE model. Loads a PyTorch model for bounding box prediction and wraps it with a FHE model using its
architecture. Converts the input image to a normalised tensor. Runs inference to predict normalised bounding box coordinates and saves the coordinates output image.
...
- Move to ml_pipeline directory: `cd engr489_project/src/fhe_face_ml_pipeline`
- Run in testing mode: `python3 fhe_fc_ml_main.py -t`

### Final

- If you are wanting to output a model then run it in "Final_Mode" and want to output/save the model.
- Move to ml_pipeline directory: `cd engr489_project/src/fhe_face_ml_pipeline`
- Run in final mode: `python3 fhe_fc_ml_main.py -f`
