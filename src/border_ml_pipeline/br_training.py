...
from br_config import (PLAIN_OPTIMIZER, PLAIN_EPOCHS, PLAIN_LEARNING_RATE, PLAIN_BATCH_SIZE, PLAIN_LOSS_FUNCTION)
...
class ML_Training_Class:
    """
    Manages the training of the ML model on grayscale image data.

    Attributes:
    optimizer (torch.optim.Optimizer): Training optimizer (e.g., Adam).
    epochs (int): Number of epochs to train.
    learning_rate (float): Learning rate for optimizer.
    batch_size (int): Batch size for DataLoader.
    loss_function (nn.Module): Loss function (e.g., MSE).
    ...
    def train_model(self, model, x_train, y_train, x_val, y_val):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_x, batch_y in train_loader:
                ...
                batch_y = batch_y.permute(0, 3, 1, 2).contiguous()
