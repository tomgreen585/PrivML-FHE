...
class ML_Main:
    ...
    def preprocess_data(self, images, train_ratio, val_ratio, seed, run_type):
        print("[INFO] Performing Preprocessing Steps on the Data")
        
        preprocess = ML_Preprocessing()
       
        x_train, y_train, x_val, y_val, x_test, y_test = preprocess.preprocessing_steps(images, train_ratio, val_ratio, seed, run_type)
    ...
    def ml_main_pipeline(self, run_type):
        ...
        if run_type == "Testing":
            ...
            self.images = self.data_loader(self.dataset_path, self.image_size)
            x_train, y_train, x_val, y_val, x_test, y_test = self.preprocess_data(self.images, self.train_ratio, self.val_ratio, self.seed, run_type)
            model = ML_Model()
