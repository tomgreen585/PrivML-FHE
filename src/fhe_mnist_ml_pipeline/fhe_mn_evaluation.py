...
from fhe_mn_config import (
    ...
    SCHEME_TYPE, ENCRYPTED_LOSS_FUNCTION, BITS_SCALE, POLY_MODULUS_DEGREE, ST_KEY_GENERATION, END_DECRYPTION_STABILITY, COEFF_MOD_BIT_SIZES,
    PLAIN_HIDDEN, PLAIN_OUTPUT, PLAIN_KERNEL_SIZE, PLAIN_STRIDE, PLAIN_PADDING, PLAIN_DROPOUT,
    SEED, AUGMENT_DATA, TRAINING_SIZE, VALIDATION_SIZE, TESTING_SIZE, ENCRYPTED_SIZE
)
...
class Ml_Metrics_Evaluation:
    ...
    def save_ml_metrics_csv():
        ...
        if os.path.isfile("fhe_mn_outputs/metrics.csv"):
            with open("fhe_mn_outputs/metrics.csv", 'r', newline='') as f:
        ...
        file_exists = os.path.isfile("fhe_mn_outputs/metrics.csv")
        with open("fhe_mn_outputs/metrics.csv", 'a', newline='') as csvfile:
    ...
    def create_ml_report(model_id):
        ...
        output_filename = f"fhe_mn_outputs/ml_metrics_report_model_{model_id}.pdf"
        ...
        elements.append(Paragraph(f"Encrypted Testing Subset Size: {int(ENCRYPTED_SIZE*100)}%", styles['Normal']))
        elements.append(Paragraph(f"Data Augmentation: {AUGMENT_DATA}", styles['Normal']))
        ...
        elements.append(Paragraph(f"Hidden Layer Size: {PLAIN_HIDDEN}", styles['Normal']))
        elements.append(Paragraph(f"Output Classes: {PLAIN_OUTPUT}", styles['Normal']))
        elements.append(Paragraph(f"Conv Kernel Size: {PLAIN_KERNEL_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Conv Stride: {PLAIN_STRIDE}", styles['Normal']))
        elements.append(Paragraph(f"Conv Padding: {PLAIN_PADDING}", styles['Normal']))
        elements.append(Paragraph(f"Dropout Rate: {PLAIN_DROPOUT}", styles['Normal']))
        elements.append(Spacer(1, 12))

        #fhe encryption settings
        elements.append(Paragraph("FHE Configuration", styles['Heading3']))
        elements.append(Paragraph(f"Encryption Scheme: {SCHEME_TYPE.name}", styles['Normal']))
        elements.append(Paragraph(f"Scale (bits): {BITS_SCALE}", styles['Normal']))
        elements.append(Paragraph(f"Polynomial Modulus Degree: {POLY_MODULUS_DEGREE}", styles['Normal']))
        elements.append(Paragraph(f"Key Generation Level: {ST_KEY_GENERATION}", styles['Normal']))
        elements.append(Paragraph(f"Decryption Stability Level: {END_DECRYPTION_STABILITY}", styles['Normal']))
        elements.append(Paragraph(f"Encrypted Loss Function: {ENCRYPTED_LOSS_FUNCTION.__class__.__name__}", styles['Normal']))
        elements.append(Paragraph(f"Coefficient Mod Bit Sizes: {COEFF_MOD_BIT_SIZES}", styles['Normal']))
        elements.append(Spacer(1, 12))
