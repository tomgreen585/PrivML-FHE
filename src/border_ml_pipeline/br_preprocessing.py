...
from br_config import (BORDER_THICKNESS, BORDER_COLOR)
...
class ML_Preprocessing:
    ...
    def generate_y_test_data(self, images, border_thickness, border_color, run_type):
        ...
        for image in images:
            ...
            bordered[:border_thickness, :, :] = border_color
            bordered[-border_thickness:, :, :] = border_color
            bordered[:, :border_thickness, :] = border_color
            bordered[:, -border_thickness:, :] = border_color
            
            self.x_data.append(original.astype(np.float32))
            self.y_data.append(bordered.astype(np.float32))
        
        self.x_data = np.stack(self.x_data)
        self.y_data = np.stack(self.y_data)
        
        if run_type == "Testing":
            x_sample = (self.x_data[1] * 255).astype(np.uint8)
            plt.imshow(x_sample)
            plt.title("Sample x_data without Border")
            plt.axis('off')
            plt.show()
            
            y_sample = (self.y_data[1] * 255).astype(np.uint8)
            plt.imshow(y_sample)
            plt.title("Sample y_data with Border")
            plt.axis('off')
            plt.show()
    ...
    def preprocessing_steps(self, images, train_ratio, val_ratio, seed, run_type):
        print("[INFO] Performing Preprocessing Steps")
        
        x_data, y_data = self.generate_y_test_data(images, self.border_thickness, self.border_color, run_type)
