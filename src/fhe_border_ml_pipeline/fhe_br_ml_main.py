import sys
from fhe_br_data_loader import ML_Data_Loader
from fhe_br_preprocessing import ML_Preprocessing
from fhe_br_plain_model import ML_Model
from fhe_br_plain_training import ML_Training_Class
from fhe_br_plain_testing import ML_Testing_Class
from fhe_br_save import ML_Saving_Model
from fhe_br_evaluation import Ml_Metrics_Evaluation
from fhe_br_fhe_model import FHE_Model
from fhe_br_enc_testing import ML_FHE_Testing_Class
...
class ML_Main:
    """
    Main pipeline handler for training the border prediction ML model.
    """
    def __init__(self):
    ...
    def data_loader(self):
        """
        Loads grayscale image data and generates input-output pairs for training/testing.

        Returns:
        x_data (np array): Original images
        y_data (np array): Bordered label images
        """
    ...
    def preprocess_data(self, x_data, y_data, run_type):
        """
        Applies preprocessing steps (e.g., normalization, train/val/test/encryption split).
        ...
        x_plain_train, y_plain_train, x_plain_val, y_plain_val, \
        x_plain_test, y_plain_test, x_enc_test, y_enc_test = preprocess.preprocessing_steps(x_data, y_data, run_type)
    ...
    def ml_testing_loop(self, model, x_test, y_test, run_type):
        ...
        Args:
        model (ML_Model): The trained model
    ...
    def ml_encrypted_testing_loop(self, plain_model, enc_model, x_test, y_test, run_type):
        """
        Tests the trained model with the initialized encrypted model on the encrypted set.

        Args:
        model (ML_Model): The trained model
        enc_model (FHE_Model): 
        x_test, y_test (np array): Test data and labels
        run_type (str): Execution context ("Testing" or "Final_Model")
        """
    ...
    def ml_main_pipeline(self, run_type):
        ...
        if run_type == "Testing":
            ...
            plain_model = self.ml_training_loop(plain_model, x_plain_train, y_plain_train, x_plain_val, y_plain_val)
            self.ml_testing_loop(plain_model, x_plain_test, y_plain_test, run_type)
