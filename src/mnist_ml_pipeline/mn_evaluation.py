...
from mn_config import (
    ...
    HIDDEN, OUTPUT, CONV_KERNEL_SIZE, CONV_STRIDE_SIZE, CONV_PADDING_SIZE, DROPOUT, POOL_KERNEL_SIZE, POOL_STRIDE_SIZE,
    SEED, AUGMENT_DATA, TRAINING_SIZE, VALIDATION_SIZE, TESTING_SIZE
)
...
class Ml_Metrics_Evaluation:
    ...
    def create_ml_report(model_id):
        ...
        elements.append(Paragraph(f"Hidden Layer Size: {HIDDEN}", styles['Normal']))
        elements.append(Paragraph(f"Output Classes: {OUTPUT}", styles['Normal']))
        elements.append(Paragraph(f"Conv Kernel Size: {CONV_KERNEL_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Conv Stride: {CONV_STRIDE_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Conv Padding: {CONV_PADDING_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Dropout Rate: {DROPOUT}", styles['Normal']))
        elements.append(Paragraph(f"Pool Kernel Size: {POOL_KERNEL_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Pool Stride: {POOL_STRIDE_SIZE}", styles['Normal']))
        elements.append(Spacer(1, 12))
