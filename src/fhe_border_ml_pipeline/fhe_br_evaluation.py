...
from fhe_br_config import (
    ...
    PLAIN_CONV1_KERNEL_SIZE, PLAIN_CONV2_KERNEL_SIZE, PLAIN_CONV1_PADDING,
...
plaintext_predicted_images = []
encrypted_predicted_images = []
...
class Ml_Metrics_Evaluation:
    ...
    def save_ml_metrics_csv():
        ...
        os.makedirs("fhe_br_outputs", exist_ok=True)
        if os.path.isfile("fhe_br_outputs/metrics.csv"):
            with open("fhe_br_outputs/metrics.csv", 'r', newline='') as f:
        ...
        file_exists = os.path.isfile("fhe_br_outputs/metrics.csv")
        with open("fhe_br_outputs/metrics.csv", 'a', newline='') as csvfile:
    ...
    def prepare_pdf_report():
        ...
        for model_name, buf in plaintext_predicted_images:
            elements.append(Paragraph(f"Plaintext: Model Prediction - {model_name}", styles['Heading3']))
            elements.append(Image(buf, width=400, height=400))
            elements.append(Spacer(1, 12))
            
        for model_name, buf in encrypted_predicted_images:
    ...
    def create_ml_report(model_id):
        ...
        os.makedirs("fhe_br_outputs", exist_ok=True)
        output_filename = f"fhe_br_outputs/ml_metrics_report_model_{model_id}.pdf"
        ...
        elements.append(Paragraph(f"Conv1 Kernel Size: {PLAIN_CONV1_KERNEL_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Conv2 Kernel Size: {PLAIN_CONV2_KERNEL_SIZE}", styles['Normal']))
        elements.append(Paragraph(f"Conv1 Padding: {PLAIN_CONV1_PADDING}", styles['Normal']))
        elements.append(Spacer(1, 12))
