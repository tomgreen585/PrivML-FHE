...
Build a CNN that performs image augmentation/generation from a supplied image from the user. Want the user to input an image of themselves and then:
...
This script performs inference using a pre-trained border detection model to add visual borders in input images. It loads a PyTroch model, processes an input image, runs the model, and saves the output.
