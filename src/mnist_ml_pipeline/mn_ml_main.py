...
from mn_config import (
    DATASET_PATH, TRAIN_RATIO, VAL_RATIO,
...
class ML_Main:
    ...
    def data_loader(self, dataset_path):
        print("[INFO] Starting to Load Data")
        
        loader = ML_Data_Loader(dataset_path)
    ...
    def preprocess_data(self, images, labels, train_ratio, val_ratio, seed, run_type):
        print("[INFO] Performing Preprocessing Steps on the Data")
        
        preprocess = ML_Preprocessing()
       
        x_train, y_train, x_val, y_val, x_test, y_test = preprocess.preprocessing_steps(images, labels, train_ratio, val_ratio, seed, run_type)
    ...
    def ml_main_pipeline(self, run_type):
        ...
        if run_type == "Testing":
            ...
            images, labels = self.data_loader(self.dataset_path)
            x_train, y_train, x_val, y_val, x_test, y_test = self.preprocess_data(images, labels, self.train_ratio, self.val_ratio, self.seed, run_type)
            model = ML_Model()
