...
from fc_config import (
    ...
    OPTIMIZER, LOSS_FUNCTION, EN_ACT, DATASET_SIZE
)
...
class Ml_Metrics_Evaluation:
    ...
    def prepare_pdf_report():
        ...
        for model_name, buf in actual_prediction:
            elements.append(Paragraph(f"Model Prediction - {model_name}", styles['Heading3']))
            elements.append(Image(buf, width=400, height=400))
            elements.append(Spacer(1, 12))
        
        for model_name, buf in actual_vs_real_prediction:
            elements.append(Paragraph(f"Predicted vs Actual - {model_name}", styles['Heading3']))
            elements.append(Image(buf, width=400, height=400))
            elements.append(Spacer(1, 12))
    ...
    def create_ml_report(model_id):
        ...
        elements.append(Paragraph(f"Activation Function: {EN_ACT.__class__.__name__}", styles['Normal']))
        elements.append(Spacer(1, 12))
