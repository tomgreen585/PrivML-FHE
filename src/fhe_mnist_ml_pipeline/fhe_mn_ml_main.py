import sys
from fhe_mn_data_loader import ML_Data_Loader
from fhe_mn_preprocessing import ML_Preprocessing
from fhe_mn_plain_model import ML_Model
from fhe_mn_plain_training import ML_Training_Class
from fhe_mn_plain_testing import ML_Testing_Class
from fhe_mn_save import ML_Saving_Model
from fhe_mn_evaluation import Ml_Metrics_Evaluation
from fhe_mn_fhe_model import FHE_Model
from fhe_mn_enc_testing import ML_FHE_Testing_Class
...
class ML_Main:
    """
    Main pipeline handler for training the mnist ML model.
    """
    def __init__(self):
        ...
        self.completed_plaintext_training_model = False
        self.completed_plaintext_testing_model = False
        self.completed_encrypted_testing_model = False
        self.completed_saving_model = False
        self.completed_ml_pipeline = False
        
    def data_loader(self):
        """
        Loads the MNIST dataset from directory
        ...
        if self.loading_dataset:
            images, labels = loader.loading_dataset()
    ...
    def preprocess_data(self, images, labels, run_type):
        """
        ...
        x_plain_train, y_plain_train, x_plain_val, y_plain_val, x_plain_test, y_plain_test, x_enc_test, y_enc_test = preprocess.preprocessing_steps(images, labels, run_type)
    ...
    def ml_plaintext_training_loop(self, model, x_train, y_train, x_val, y_val):
        ...
        """
        print("[INFO] Starting ML Plaintext Training Loop")
    ...
    def ml_plaintext_testing_loop(self, model, x_test, y_test, run_type):
        ...
        """
        print("[INFO] Starting ML Plaintext Testing Loop")
    ...
    def ml_encrypted_testing_loop(self, plain_model, enc_model, x_test, y_test):
        """
        Tests the trained model with the intiialized encrypted model on the encrypted set.

        Args:
        model (ML_Model): trained model
        enc_model (FHE_Model): initialized fhe model
        x_test, y_test (np.ndarray): encrypted data and labels
        run_type (str): Execution context ("Testing" or "Final_Model")
        """
        ...
        enc_testing.testing_pipeline(plain_model, enc_model, x_test, y_test)
    ...
    def ml_main_pipeline(self, run_type):
        ...
        if run_type == "Testing":
            ...
            images, labels = self.data_loader()
            x_plain_train, y_plain_train, x_plain_val, y_plain_val, x_plain_test, y_plain_test, x_enc_test, y_enc_test = self.preprocess_data(images, labels, run_type)
            ...
            self.ml_encrypted_testing_loop(plain_model, fhe_model, x_enc_test, y_enc_test)
