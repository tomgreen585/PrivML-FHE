...
validation, testing and encryption datasets. Preprocesseng stacked to shape (N, 96, 96, 3) -> (N, Height, width, channel).
...
class ML_Preprocessing:
    ...
    encrypted_size (float): size of dataset for encryption
    ...
    def perform_dataset_split(self, x_data, y_data):
        """
        Splits the dataset into training, validation, testing and encryption sets.
        ...
        Returns:
        (x_train, y_train, x_val, y_val, x_test, y_test, x_enc, y_enc)
        """
        ...
        test_enc_size = int(enc_test * dataset_size)
        ...
        test_enc_set = dataset[dataset_start:dataset_start+test_enc_size]
        dataset_start += test_enc_size
        ...
        #encrypted datasets
        x_enc_test = x_data[test_enc_set]
        y_enc_test = y_data[test_enc_set]
    ...
    def preprocessing_steps(self, images, labels, run_type):
        ...
        if `run_type` is "Testing". Performs dataset augmentation. Splits data into train/val/test/encrypted sets
        ...
        Returns:
        training, validation, test and encrypted sets
        """
        ...
        if self.augment_data:
            ...
            x_plain_test, y_plain_test, x_enc_test, y_enc_test = self.perform_dataset_split(all_images, all_labels)
        else:
            x_plain_train, y_plain_train, x_plain_val, y_plain_val, \
            x_plain_test, y_plain_test, x_enc_test, y_enc_test = self.perform_dataset_split(images, labels)
